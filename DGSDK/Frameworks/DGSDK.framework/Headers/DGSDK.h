...
#import "DGSDKDefines.h"
#import "DGUserModel.h"

@interface DGSDK : NSObject
...
- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret callback:(DGCallbackBlock)callback;

- (void)setGameID:(NSString *)gameID sdkSecret:(NSString *)sdkSecret isShowLoading:(Boolean)isShowLoading callback:(DGCallbackBlock)callback;
 ...
 *  @brief 切换系统语言
 *
 *  @param languageType 语言类型，DGSDKDefines中有声明
 */
- (void)setLanguage:(DGLanguageType)languageType;
 ...
 *
 */
- (void)updateCallback:(DGUpCallback)block;
 ...
 *
 */
- (void)login:(DGCallbackBlock)block;

- (void)login:(Boolean)isCanClose andBlock:(DGCallbackBlock)block;

/*
 * @brief 用户登录失效的回调，包括注销，用户中心，cookie失效等情况的回调
 */
- (void)logoutCallback:(DGCallbackBlock)Block;

/**
 *  @brief 用户点击退出，不会跳转到登录页面
 *
 */
- (void)logout:(DGCallbackBlock)Block;
 ...
 */
- (void)userInfo:(DGCallbackBlock)block;
 ...
 *  @param  block 回调block
 */
- (void)showGameManager:(NSString *)title block:(DGCallbackBlock)block;
 ...
 *
 */
- (void)isBindEmail:(DGCallbackBlock)block;

@end

#pragma mark - 内购 **************************************************

@interface DGSDK(DGIAP)
...
- (void)buyWithProductId:(NSString *)productId
           ...
           callbackBlock:(DGCallbackBlock)block ;
